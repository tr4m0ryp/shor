...
const InjectionQueueSchema = z.object({
	vulnerabilities: z.array(InjectionVulnerability),
});
const XssQueueSchema = z.object({ vulnerabilities: z.array(XssVulnerability) });
const AuthQueueSchema = z.object({
	vulnerabilities: z.array(AuthVulnerability),
});
const SsrfQueueSchema = z.object({
	vulnerabilities: z.array(SsrfVulnerability),
});
const AuthzQueueSchema = z.object({
	vulnerabilities: z.array(AuthzVulnerability),
});
...
function toOutputFormat(zodSchema: z.ZodType): JsonSchemaOutputFormat {
	return {
		type: "json_schema",
		schema: z.toJSONSchema(zodSchema, { target: "draft-07" }) as Record<
			string,
			unknown
		>,
	};
...
const VULN_AGENT_OUTPUT_FORMAT: Partial<
	Record<AgentName, JsonSchemaOutputFormat>
> = {
