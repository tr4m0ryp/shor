 ...
 * Uses custom outputPath if provided, otherwise nests by category under WORKSPACES_DIR
 */
export function generateAuditPath(sessionMetadata: SessionMetadata): string {
	...
	if (sessionMetadata.outputPath) {
		return path.join(sessionMetadata.outputPath, sessionIdentifier);
	}
	return workspaceDir(sessionIdentifier);
}
 ...
 */
export function generateSessionJsonPath(
	sessionMetadata: SessionMetadata,
): string {
	const auditPath = generateAuditPath(sessionMetadata);
 ...
 */
export function generateWorkflowLogPath(
	sessionMetadata: SessionMetadata,
): string {
	const auditPath = generateAuditPath(sessionMetadata);
 ...
 */
export async function initializeAuditStructure(
	sessionMetadata: SessionMetadata,
): Promise<void> {
	const auditPath = generateAuditPath(sessionMetadata);
