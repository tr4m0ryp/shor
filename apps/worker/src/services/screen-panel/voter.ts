...
export function voterFramingBlock(args: {
	...
	const template = [
		...
		"surface in the <target> block above (an API typically lives on a distinct port,",
		"e.g. the `:8080` origin) — NOT the primary SPA origin. If `{{WEB_URL}}/<path>`",
		"returns the SPA shell / index.html / a static 404-405, you are on the wrong",
		"origin; switch to the API origin and retry the same path there.",
