...
/**
 * Worker-side network address checks (LAUNCH-SPEC §5.6, §3.3).
 *
 * Self-contained copy of the dashboard's `guardrails/net.ts` — the two packages
 * MUST NOT import each other, and the shape is the contract. Blocks the cloud
 * metadata endpoint and internal/loopback/link-local ranges (SSRF defense in
 * depth; gVisor blocks the metadata IP at the sandbox boundary too).
 */
...
export const METADATA_HOSTNAMES: readonly string[] = ["metadata.google.internal", "metadata"];
...
const BLOCKED_V4: readonly Cidr[] = BLOCKED_V4_CIDRS.map(parseCidr).filter(
	(c): c is Cidr => c !== null,
);
